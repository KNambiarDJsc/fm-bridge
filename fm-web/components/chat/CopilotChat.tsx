...
export function CopilotChat({ onClose }: { onClose: () => void }) {
    ...
    return (
        <div className="flex flex-col h-full bg-bg2 border-t border-b">
            {/* Header */}
            <div className="flex items-center gap-2 px-4 py-3 border-b border-b shrink-0">
                ...
                <span className="font-mono text-[9px] text-t3 ml-1">— intelligence access layer, not the product</span>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3 min-h-0">
                    ...
                    <div key={i} className={cn("flex", m.role === "user" ? "justify-end" : "justify-start")}>
                        <div className={cn(
                            ...
                            m.role === "user"
                                ...
                                : "bg-bg3 text-t2 border border-b"
                        )}>
                            {m.content}
                        </div>
                    </div>
                ...
                {isPending && (
                    <div className="flex justify-start">
                        <div className="bg-bg3 border border-b rounded-lg px-3 py-2 font-mono text-[12px] text-t3">
            ...
            <div className="flex gap-2 p-3 border-t border-b shrink-0">
                <input
                    ...
                    className="flex-1 bg-bg3 border border-b rounded-md px-3 py-2 font-mono text-[12px] text-t1 placeholder-t3 outline-none focus:border-bl/50 transition-colors"
                />
                <button
                    onClick={() => send(input)}
